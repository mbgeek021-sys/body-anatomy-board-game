...
if (!window.__bodyGameClickBound) {
  window.__bodyGameClickBound = true;
  ...
  document.addEventListener('click', e => {
    ...
    if (actionEl) {
      ...
      if (action === 'roll') {
        window.runSafe(() => window.handleRoll(), 'Roll failed.');
        return;
      }

      if (action === 'reset') {
        window.runSafe(() => window.resetGame(), 'Reset failed.');
        return;
      }

      if (action === 'share') {
        window.runSafe(() => window.copyShareLink(), 'Share failed.');
        return;
      }
    }

    const triviaEl = e.target.closest('[data-trivia-choice]');
    if (triviaEl) {
      window.submitTrivia(triviaEl.getAttribute('data-trivia-choice'));
    }
  });
}
