  ...
  return `
    <div class="topbar">
      ...
      <div class="chip-row">
        ...
        <div class="chip alt">${window.getPlayerCountText()}</div>
        <div class="chip">${window.escapeHtml(window.getConnectionText())}</div>
        <button class="btn btn-small btn-white" id="audioToggleBtn" type="button">
          ${state.audioMuted ? '🔇 Audio' : '🔊 Audio'}
        </button>
        <button class="btn btn-small btn-white" id="backToLobbyBtn" type="button">Back</button>
      </div>
    </div>
  `;
};
      ...
      <div class="lobby-wrap">
        <div class="lobby-left">
          <div class="brand">
            ...
            <div>
              <div class="domain">${window.escapeHtml(window.APP_CONFIG?.SITE_DOMAIN || 'bodyanatomyboardgame.com')}</div>
              <div class="title">${window.escapeHtml(window.APP_CONFIG?.GAME_TITLE || 'Body Anatomy Board Game')}</div>
              <div class="subtitle">
                ${window.escapeHtml(window.APP_CONFIG?.SUBTITLE || '')}<br>
                ${window.escapeHtml(window.APP_CONFIG?.SCHOOL || '')}
              </div>
...
window.renderTurnCard = function(){
  ...
  return `
    <div class="hud-card">
      ...
      <div class="turn-hero">
        <div style="font-size:13px;opacity:.92;">${yourTurn ? 'Your turn' : 'Now studying'}</div>
        <h3>${window.escapeHtml(current ? window.getPlayerName(current) : 'Player')}</h3>
        <p>Last Roll: ${window.escapeHtml(state.lastRoll ?? '-')}</p>
      </div>
    </div>
  `;
};
    ...
    <div class="hud-card">
      ...
      <div class="controls-grid">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
          <button id="rollBtn" class="btn btn-teal" type="button" ${state.isRolling ? 'disabled' : ''}>
